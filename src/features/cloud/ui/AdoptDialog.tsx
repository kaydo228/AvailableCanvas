 ...
 * стартовать, пока первый ещё не прочитал список локальных досок, и тогда
 * оба круга решали выгружать одну и ту же доску независимо друг от друга —
 * двойной `upsert`.
 *
...
export function AdoptDialog() {
  ...
  const { ids, owner, email, cycle } = question;

  // Оба ответа делают одно и то же после своего действия: пишут решение
  // сразу (человеку не нужно ждать первый круг, чтобы получить отклик),
  // затем ЖДУТ `cycle` — первый круг обязан полностью завершиться, прежде
  // чем второй прочитает локальные доски заново, иначе оба решат выгружать
  // одну и ту же доску независимо (раунд правок 1).
