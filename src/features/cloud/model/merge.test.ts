...
describe('decide', () => {
  ...
  it('была на сервере, но есть невыгруженные правки — НЕ удаляем', () => {
    // Ответ сервера «доски нет» встречается и тогда, когда доску удалили
    // с другого устройства, и тогда, когда её просто не успели выгрузить.
    // Правки, которые ещё не уехали, есть только здесь — снести их нечем
    // будет восстановить, поэтому такую доску оставляем как есть.
    const out = decide([local('a', 100, { owner: ME, remoteUpdatedAt: 50, dirty: true })], [], ME);
    expect(out).toEqual([{ kind: 'nothing', projectId: 'a' }]);
  });
