...
test('NFR-03: сохранение доски на 1000 узлов не блокирует главный поток', async ({ page }) => {
  ...
  const measurement = await page.evaluate(async () => {
    ...
    // Ждём, пока осядет открытие документа.
    ...
    // Счётчик кадров: самый большой разрыв между кадрами — это и есть «подвисло».
    ...
    // Правка, запускающая автосохранение всей доски.
  ...
  // 50 мс — порог longtask и примерно тот предел, за которым набор начинает
  // ощущаться рваным. Больше — переделывать запись.
