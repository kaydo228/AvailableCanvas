...
test('после загрузки и правок доска не ходит в сеть (критерий 12)', async ({ page, baseURL }) => {
  ...
  // Исключения больше нет. Долг по Google Fonts закрыт 28 августа: ссылки
  // из index.html убраны, шрифты берутся системные — стек в --font-sans
  // и --font-mono уже содержал полные фолбэки. Теперь наружу не уходит
  // вообще ничего, и проверка это требует буквально.
  ...
  // Ничего наружу не уходит — ни доски, ни картинок, ни телеметрии, ни шрифтов.
  // Это и есть смысл критерия 12.
