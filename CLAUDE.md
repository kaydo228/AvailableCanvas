...
Чего нет: **валидатора чужого документа**. `src/shared/model/invariants.ts`
заглушен через `notImplemented`, поэтому **13 тестов красные**. Это не «осознанное
TDD-красное на будущее», а конкретная незакрытая задача: документ из IndexedDB
попадает в стор без единой проверки, и через это достижимы битые данные —
см. `docs/nightly/connectors/08-аудит-ломатель.md`.
