...
**Известная дыра (находка ломателя F-3 #5, не починена):** `clampZoom` живёт
внутри `zoomAt`, а `setViewport` его не зовёт — `setViewport({zoom: 99})`,
`-1`, `NaN` проходят. Публичное действие общего шва мимо инварианта. Правишь
зум — правь и здесь. Полный список из 11 находок — `docs/nightly/breaker/99-отчёт.md`.
