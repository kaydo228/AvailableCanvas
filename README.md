...
| Целостность данных | валидатор `shared/model/invariants.ts` находит нарушения всех пяти инвариантов, `features/persistence/repair.ts` чинит документ **на обоих входах** — при импорте файла и при чтении из IndexedDB, поэтому открытие доски задним числом лечит то, что испортилось раньше |
...
| Не сделано | Почему |
|---|---|
| Копирование и вставка объектов, `Cmd+C` / `Cmd+V` | строки в таблице клавиш есть, действия за ними нет: `run` не назначен, нажатие ничего не делает и в справке помечено «пока не готово». Дублирование `Cmd+D` закрыло основную потребность, буфер обмена **между досками** остался. `Cmd+V` работает только для картинки из системного буфера |
...
Все **536 юнит-тестов** и все **53 сценария Playwright** — зелёные;
замер производительности вынесен в отдельный прогон `npm run perf`
(три сценария, в один поток) и тоже зелёный: 55 fps на тысяче узлов.
...
**Про распределение коммитов, чтобы вопрос не возник.** `git log` даёт
64 коммита у `roman_restov`, 12 у `alfis`, 2 у `kaydo`, и это не пропорция
вклада. Зона B делалась на ветке `setup/tooling` крупными блоками — там
меньше коммитов, но в них экран проектов, хранение, инспектор, история,
экспорт и вся обвязка. Плюс два перекоса в пользу первой цифры: сведение
двух разъехавшихся веток 27 августа и все доработки последнего дня
(группы, дублирование, поворот, размер, инварианты, worktrees, изоляция,
инъекция) писались с одной машины. Журнал сессий у `alfis` заканчивается
26 августа по той же причине.
