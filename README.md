...
| FR-11 Сохранение | автосохранение с дебаунсом 500 мс, индикатор «Сохранение…» → «Все изменения сохранены», положение вида переживает перезагрузку. С аккаунтом добавляется третье, спокойное состояние — «Сохранено только здесь»: доска цела в браузере, просто ещё не уехала на сервер. Сохранение доски на 1000 узлов не блокирует ввод |
...
| Синхронизация *(сверх ТЗ)* | вход по почте и паролю (Supabase Auth), выгрузка досок и картинок при каждом сохранении, скачивание на другом устройстве, слияние по правилу «побеждает более поздняя правка». Публичная ссылка на чтение — доска открывается без входа, без панели инструментов и без инспектора; чем она не является — в «Чего не умеет» ниже |
...
| Бесплатный проект Supabase засыпает | без единого запроса неделю — просыпается сам на первом же обращении, но с задержкой в несколько секунд. Кто заметит первым — это тот, кто откроет доску после долгого перерыва. Лечится пингом раз в день, инструкция и пример GitHub Action — в [docs/cloud-setup.md](docs/cloud-setup.md) |
...
Все **588 юнит-тестов** и **80 сценариев Playwright** — зелёные. `npm run check`
