...
| Известные дефекты из прогона агента-ломателя (F-3) | 11 находок от независимого проверяющего, 28 августа: две ведут к потере данных обычным жестом (коннектор на удалённый узел, необратимое сжатие группы), одна обходит инвариант зума. Сведены в [docs/nightly/breaker/99-отчёт.md](docs/nightly/breaker/99-отчёт.md), разобраны в [REPORT.md](REPORT.md). Ни одна пока не починена — отдельным циклом после сдачи |
...
Все **536 юнит-тестов** и все **55 сценариев Playwright** — зелёные;
...
| **kaydo** | владелец репозитория | приём PR из `setup/tooling` |
